...
class WindowVisionTransformer(nn.Module):
    ...
    def forward(self, x: torch.Tensor, window_masks=None):
        if self.input_patchnorm:
            x = x.reshape(x.shape[0], x.shape[1], self.grid_size[0], self.patch_size[0], self.grid_size[1],
                          self.patch_size[1])
            x = x.permute(0, 2, 4, 1, 3, 5)
        ...
        if window_masks:
            for mask in window_masks:
                ...
                tokens_list.append(pooled.reshape((mask_num, 1, 1, -1)).permute(1, 0, 2, 3))
        ...
        return tokens_list, tokens, pooled
